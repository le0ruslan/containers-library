#ifndef S21Containers_SRC_S21_MULTISET_H_
#define S21Containers_SRC_S21_MULTISET_H_
...
class Multiset {
 public:
  ...
  Multiset();
  Multiset(std::initializer_list<key_type> const& data);
 ...
 public:
  ...
  // additional functions other than Set
...
Multiset<key_type>::Multiset() : size_(0), root_(nullptr) {}

template <typename key_type>
Multiset<key_type>::Multiset(std::initializer_list<key_type> const& data)
...
template <typename key_type>
Multiset<key_type>::Multiset(const Multiset& other) : size_(0), root_(nullptr) {
...
template <typename key_type>
Multiset<key_type>& Multiset<key_type>::operator=(Multiset&& other) noexcept {
  if (this == &other) return *this;
...
template <typename key_type>
typename Multiset<key_type>::size_type Multiset<key_type>::MaxSize() {
...
void Multiset<key_type>::Merge(Multiset& other) {
  ...
  while (it != other.End()) {
    auto next_it = it;
    ++next_it;
    auto result = Insert(std::move(*it));
...
template <typename key_type>
Multiset<key_type>::iterator::iterator() : node_(nullptr) {}

template <typename key_type>
Multiset<key_type>::iterator::iterator(Node* node) : node_(node) {}

template <typename key_type>
typename Multiset<key_type>::key_type Multiset<key_type>::iterator::operator*()
    const {
  if (node_ == nullptr)
...
Multiset<key_type>::iterator::operator++() {
...
template <typename key_type>
typename Multiset<key_type>::iterator Multiset<key_type>::iterator::operator++(
    int) {
  auto tmp = *this;
  operator++();
  return iterator(tmp);
...
template <typename key_type>
bool Multiset<key_type>::iterator::operator==(const iterator& other) const {
...
template <typename key_type>
typename Multiset<key_type>::iterator Multiset<key_type>::Begin() const {
...
template <typename key_type>
typename Multiset<key_type>::iterator Multiset<key_type>::End() const {
...
template <typename key_type>
typename Multiset<key_type>::iterator Multiset<key_type>::LowerBound(
    const key_type& key) {
  Node* tmp = root_;
  Node* result = nullptr;
...
template <typename key_type>
typename Multiset<key_type>::iterator Multiset<key_type>::UpperBound(
    const key_type& key) {
  Node* tmp = root_;
  Node* result = nullptr;
  while (tmp != nullptr) {
...
std::pair<typename Multiset<key_type>::iterator,
          typename Multiset<key_type>::iterator>
Multiset<key_type>::EqualRange(const key_type& key) {
  auto lower = LowerBound(key);
  auto upper = UpperBound(key);
...
Multiset<key_type>::InsertMany(Args&&... args) {
  std::vector<std::pair<iterator, bool>> result;
  (result.emplace_back(this->Insert(std::forward<Args>(args)), true), ...);
  return result;
}

template <typename key_type>
void Multiset<key_type>::PrintInor() {
  Node* tmp = root_;
  this->Inorder(tmp);
}

template <typename key_type>
void Multiset<key_type>::PrintPost() {
  Node* tmp = root_;
  this->Post(tmp);
}

template <typename key_type>
void Multiset<key_type>::PrintPre() {
  Node* tmp = root_;
  this->PrePrint(tmp);
}

template <typename key_type>
void Multiset<key_type>::PrePrint(Node* tmp) {
  if (tmp == nullptr) return;
  std::cout << tmp->data_ << ' ';
...
template <typename key_type>
void Multiset<key_type>::Pre(Node* tmp) {
  if (tmp == nullptr) return;
...
template <typename key_type>
void Multiset<key_type>::Post(Node* tmp) {
  if (tmp == nullptr) return;
  this->Post(tmp->left_);
...
template <typename key_type>
void Multiset<key_type>::Inorder(Node* tmp) {
  if (tmp == nullptr) return;
...
template <typename key_type>
void Multiset<key_type>::Postorder(Node* tree) {
...
template <typename key_type>
typename Multiset<key_type>::Node* Multiset<key_type>::Grandparent(Node* tmp) {
...
template <typename key_type>
typename Multiset<key_type>::Node* Multiset<key_type>::Uncle(Node* tmp) {
...
template <typename key_type>
void Multiset<key_type>::RotateLeft(Node* tmp) {
...
template <typename key_type>
void Multiset<key_type>::RotateRight(Node* tmp) {
...
template <typename key_type>
void Multiset<key_type>::InsertCase1(Node* tmp) {
  if (tmp->parent_ == nullptr) {
    tmp->color_ = Black;
  } else
...
template <typename key_type>
void Multiset<key_type>::InsertCase2(Node* tmp) {
...
template <typename key_type>
void Multiset<key_type>::InsertCase4(Node* tmp) {
  Node* g = this->Grandparent(tmp);
...
template <typename key_type>
void Multiset<key_type>::InsertCase5(Node* tmp) {
  Node* g = this->Grandparent(tmp);
  tmp->parent_->color_ = Black;
  g->color_ = Red;
...
template <typename key_type>
typename Multiset<key_type>::Node* Multiset<key_type>::Sibling(Node* tmp) {
...
template <typename key_type>
void Multiset<key_type>::ReplaceNode(Node* tmp, Node* child) {
  if (tmp->parent_ == nullptr)
    root_ = child;
  else {
    if (tmp == tmp->parent_->left_)
...
template <typename key_type>
void Multiset<key_type>::DeleteOneChild(Node* tmp) {
...
template <typename key_type>
bool Multiset<key_type>::IsLeaf(Node* tmp) {
  return tmp != nullptr && tmp->left_ == nullptr && tmp->right_ == nullptr;
}

template <typename key_type>
void Multiset<key_type>::DeleteCase1(Node* tmp) {
  if (tmp != nullptr && tmp->parent_ != nullptr) this->DeleteCase2(tmp);
}

template <typename key_type>
void Multiset<key_type>::DeleteCase2(Node* tmp) {
...
template <typename key_type>
void Multiset<key_type>::DeleteCase3(Node* tmp) {
  Node* s = this->Sibling(tmp);
...
template <typename key_type>
void Multiset<key_type>::DeleteCase4(Node* tmp) {
  Node* s = this->Sibling(tmp);
...
template <typename key_type>
void Multiset<key_type>::DeleteCase5(Node* tmp) {
  Node* s = this->Sibling(tmp);
  if (s->color_ == Black) {
    if ((tmp == tmp->parent_->left_) &&
        (s->right_ == nullptr || s->right_->color_ == Black) &&
...
template <typename key_type>
void Multiset<key_type>::DeleteCase6(Node* tmp) {
  Node* s = this->Sibling(tmp);
...
#endif  // S21Containers_SRC_S21_MULTISET_H_
